...
- **内**:声明式编码表 → 生成 C++ 解码器 + Python 汇编 API;标量 + warp + ITS 执行核;内存空间(global/shared/local/const)与 barrier;MMA(bit-exact)、SFU、归约、async-copy/TMA(eager);kernel-builder;验证框架。
...
- **③ 内存与同步**:memory_space(global/shared/local/const)+ barrier + atomics。
  验收:shared-mem reduction、barrier 类测试结果对；const 为只读第四空间，通过 `LDC` 与 launch 注入访问，snapshot 使用顶层 `const_memory`，`memory` 子树仍保持 global/shared/local 三键。
