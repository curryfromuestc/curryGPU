...
- **内存** = 抽象 `memory_space` 基类 + `memory_space_impl<4KB 块 hashmap>`（稀疏），per-space；global/shared/local 是可写空间，const 是只读第四空间并由 launch 注入；`load<T>/store<T>` 包成 32-地址 gather/scatter，支持 byte-masked 写；atomic RMW 单线程序列化、不可分。
...
block_state_t (CTA;由 cluster_state 持有)
  ...
  ├─ const_memory:  memory_space_impl          // grid-level readonly banks;serialized as top-level const_memory
